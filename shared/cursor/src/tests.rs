...
fn split_at_basic() {
    ...
    let (left, right_cursor) = cursor.split_at(3);
...
#[test]
fn split_at_checked_success() {
    let mut data = [0u8; 8];
    let mut cursor = Cursor::new(&mut data);
    cursor.try_set_position(2).unwrap();

    let (left, right_cursor) = cursor.split_at_checked(5).unwrap();
...
#[test]
fn split_at_checked_same_position() {
    let mut data = [0u8; 4];
    let cursor = Cursor::new(&mut data);

    let (left, right_cursor) = cursor.split_at_checked(0).unwrap();
...
#[test]
fn split_at_checked_out_of_bounds() {
    let mut data = [0u8; 5];
    let cursor = Cursor::new(&mut data);

    assert!(cursor.split_at_checked(6).is_none());
}

#[test]
fn split_at_checked_overflow_limit() {
    let mut data = [0u8; 10];
    let mut cursor = Cursor::new_with_limit(&mut data, 5);
...
fn split_at_panics_on_overflow() {
    let mut data = [0u8; 5];
    let cursor = Cursor::new(&mut data);

    // Should panic due to out-of-bounds
    let _ = cursor.split_at(6);
}
...
fn split_cursor_write_on_right() {
    let mut data = [0u8; 6];
    let cursor = Cursor::new(&mut data);
    let (_left, mut right) = cursor.split_at(3);
...
fn test_multiple_sequential_splits() {
    ...
    let (left1, cursor) = cursor.split_at(3);
    let (left2, cursor) = cursor.split_at(2);
    let (left3, right)   = cursor.split_at(2);
...
fn test_split_then_write_on_each_segment() {
    ...
    let (seg1, cursor) = cursor.split_at(4);
    let (seg2, mut cursor) = cursor.split_at(3);
...
fn test_back_to_back_splits_exhausting_buffer() {
    ...
    let (a, cursor) = cursor.split_at(2);
    let (b, cursor) = cursor.split_at(2);
    let (c, cursor) = cursor.split_at(2);
...
fn test_split_at_cursor_position_then_write() {
    ...
    let (left, mut right) = cursor.split_at(4);
...
fn test_split_with_zero_length_remaining() {
    ...
    let (left, right) = cursor.split_at(3);
    assert_eq!(left, &[1, 2, 3]);
...
fn test_split_reuse_then_backtrack_position() {
    ...
    let (_a, mut cursor_b) = cursor.split_at(3);
